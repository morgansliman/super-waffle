...
function formCtrl ($scope, $http) {
  $scope.coin1 = 'Bitcoin - BTC'
  $scope.coinList = ['Ether - ETH', 'Litecoin - LTC', 'Dash - DASH']
  ...
  function getRate (coin1, coin2, amount) {
    $http({
      ...
      var highest = 0
      var best = ''
      for (let market in data.data) {
        console.log(market)
        console.log(data.data[market])
        if (data.data[market] > highest) {
          best = market
          highest = data.data[market]
        }
      }
      $scope.trades.push({
        coin1: coin1.split(' - ')[1],
        ...
        unit1: coin1.split(' - ')[0],
        unit2: coin2.split(' - ')[0],
        marketResults: data.data,
        bestRate: {},
        amount: amount,
        highest: highest
      })
      $scope.trades[$scope.trades.length - 1].bestRate[best] = { 'background-color': '#00ee22' }
    }).catch(function (err) {
      console.log(err)
    })
  }
